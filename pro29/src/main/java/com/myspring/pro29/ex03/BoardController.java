...
public class BoardController {
	...
	//특정값 
	@RequestMapping(value = "/{aricleNO}", method = RequestMethod.GET)
	public ResponseEntity<ArticleVO> findArticle(@PathVariable("aricleNO") Integer articleNo){
	...
	public ResponseEntity<String> addArticle(@RequestBody ArticleVO articleVO){
		...
		try {
			...
			resEntity = new ResponseEntity<String>("SUCCEEDED", HttpStatus.OK);
		}catch(Exception e) {
	...
	public ResponseEntity<String> modArticle(@PathVariable Integer articleNO, @RequestBody ArticleVO articleVO){
		ResponseEntity<String> resEntity = null;
		try {
	...
	public ResponseEntity<String> removeArticle(@PathVariable Integer articleNO){
		ResponseEntity<String> resEntity = null;
		try {
			logger.info("modArticle 메서드 호출");
